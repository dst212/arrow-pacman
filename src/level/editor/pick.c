...
bool levelPickColor(WINDOW*w, const short y, const short x, uint8_t*color) {
	int ch, i;
	short cx;
...
void levelPickWall(WINDOW*w, Level*l, const short y, const short x) {
	int i, j, ch;
	WINDOW*sidewin;
...
void levelPickSymbol(WINDOW*w, wchar_t*symbol, const uint8_t color) {
	int i, j, ch;
	short x, y;
	WINDOW*sidewin;
	bool refreshList = true; //refresh the wall list
...
void levelChangeSprites(WINDOW*w, const short y, const short x, const int rows, const int cols, wchar_t table[][4], uint8_t color[], const int defColor) {
	int i = 0, j = 0, ch;
	do {
